...
# working on: 10/30
# TODO: 2D plotting (then project axes)
...
class CrockerSignals(object):
    ...
    def __init__(self, filename):  # cherenkov or lfs_en
        ...
        # 12.81
        self.cable_delays = {1: 0, 2: 6.58, 3: 0.7, 4: 25.1}  # 25.1 used  for everything but plotting. 12.81 otherwise
    ...
    def _lfs_energy_signal(self, time_calibrated_bins, voltage_calibrations, method="peak", delay_corrected=False):
        ...
        # det_name = "lfs_en"
        det_name = self.det_type
        det_time_bins = time_calibrated_bins[self.ch_names[det_name]]
        self.buffer[:] = voltage_calibrations[self.ch_names[det_name]]
        # ds = delay_corrected * self.cable_delays[self.ch_names["lfs_en"]]
        ds = delay_corrected * self.cable_delays[self.ch_names[self.det_type]]
    ...
    def lfs_energy_spectrum(self, method="peak", bins=None, log_scale=False):
    ...
    def test_rf_t0_points(self, delay=False):
        ...
        labels = ["RF", "LFS fast", "LFS slow", "T0 (inv.)"]
    ...
    def lfs_2D_energy_time_plots(self, method="integral", delay_correct=True):
    ...
    def t0_signal_height(self, output=False, suppress_plots=False, output_fname=None):
        """Generates histogram t0 signal amplitude."""
        board = self.board_ids[0]
        channels = self.channels[board]
        t0_frac = np.array([0.2])  # "CFD" for t0
        ...
        if output_fname is None:
            ...
            output_fname = os.path.splitext(self.filename)[0] + "t0max"

        print("OUTPUT TRUE")
        vals = {'bins': bins, 'counts': amps}
        print("vals: ", vals)
        np.savez(output_fname, filename=self.filename,
                 bins=bins, counts=amps)


def test_triggers(fname, det_type="lfs_en"):  # no det field, only LFS files here
    tst = CrockerSignals(fname)
    if det_type != "lfs_en":
        tst.det_type = "cherenkov"
        tst.ch_names = {"rf": 1, "lfs": 2, "cherenkov": 3, "t0": 4}
...
def full_plotting(fname):
    ...
    lfs_data.lfs_2D_energy_time_plots(method=method, delay_correct=delay_correct)


def t0_heights(fname, det_type="lfs_en", output=False, suppress_plots=False):
    lfs_data = CrockerSignals(fname)
    if det_type != "lfs_en":
        lfs_data.det_type = "cherenkov"
        lfs_data.ch_names = {"rf": 1, "lfs": 2, "cherenkov": 3, "t0": 4}
    print(lfs_data.f.board_ids)
    lfs_data.t0_signal_height(output=output, suppress_plots=suppress_plots)
...
def main():
    ...
    # det = "cherenkov"
    # data_file_name = "20221017_Crocker_31.6V_LFS_500pa_SingleDataset_nim_amp_p0_v20.dat"  # p0 LFS
    # data_file_name = "20221017_Crocker_31.6V_LFS_500pa_SingleDataset_nim_amp_p2_v19.dat"  # p2 LFS
    # data_file_name = "20221017_Crocker_31.6V_LFS_500pa_SingleDataset_nim_amp_p4_v18.dat"  # p4 LFS
    # data_file_name = "20221017_Crocker_31.6V_LFS_500pa_DualDataset_nim_amp_p5_v17.dat"   # p5 LFS
    # data_file_name = "20221017_Crocker_31.6V_LFS_500pa_DualDataset_nim_amp_p6_v16.dat"  # p6 LFS
    ...
    # ONLY USE WITH t0_heights
    data_file_name = "20221017_Crocker_31.6V_cherenkov_400pa_DualDataset_v4.dat"
    ...
    # test_triggers(fname)  # default
    # test_triggers(fname, det_type="cherenkov")
    # t0_heights(fname)
    t0_heights(fname, det_type="cherenkov")
    # t0_rf_det_delta_t(fname)  # 1d time plots
